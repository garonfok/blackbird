...
import { pushInstrument, removeInstrument } from "@/routes/Dashboard/reducers/filterSlice";
...
export function Instruments() {
  ...
  async function handleChange(event: ChangeEvent<HTMLInputElement>, instr: Instrument) {
            ...
            <div className="max-h-[300px]">
              {instruments.map(instrument => (
                <CommandItem key={instrument.id} asChild>
                  <label htmlFor={instrument.id.toString()} className="flex items-center gap-[8px] group">
                    <input
                      onChange={(event) => handleChange(event, instrument)}
                      id={instrument.id.toString()}
                      checked={filter.instruments.some(i => i.id === instrument.id)}
                      type="checkbox"
