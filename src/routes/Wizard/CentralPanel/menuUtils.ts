...
type PartScoreType = z.infer<typeof partFormSchema> | z.infer<typeof scoreFormSchema>;

export function clearFile(id: number, type: ItemType, pieceForm: UseFormReturn<z.infer<typeof pieceFormSchema>>) {
  pieceForm.setValue(
    type,
...
export function removeItem(id: number, type: ItemType, pieceForm: UseFormReturn<z.infer<typeof pieceFormSchema>>, checkedMap: Map<string, CheckedState>, setCheckedMap: Dispatch<SetStateAction<Map<string, CheckedState>>>) {
  pieceForm.setValue(
    type,
...
export function duplicate(id: number, type: ItemType, pieceForm: UseFormReturn<z.infer<typeof pieceFormSchema>>) {
  const item = pieceForm.getValues(type).find((item) => item.id === id)!;
  const index = pieceForm.getValues(type).map((item) => item.id).indexOf(id);
  const newItem = {
    ...item,
    id: Math.max(...pieceForm.getValues(type).map((item: PartScoreType) => item.id), 0) + 1,
  };
...
export function moveAbove(ids: number[], type: ItemType, pieceForm: UseFormReturn<z.infer<typeof pieceFormSchema>>) {
...
export function moveBelow(ids: number[], type: ItemType, pieceForm: UseFormReturn<z.infer<typeof pieceFormSchema>>) {
  const items = pieceForm.getValues(type);
  const selectedItems = items.filter((item) => ids.includes(item.id));
  const unselectedItems = items.filter((item) => !ids.includes(item.id));
